...
weightdiff_data = donnee %>%
  select(DifferencePoids,C_over_N, TeaType, NitrogenType, DoseN, Producteur, Champs, DoseRep) %>%
  drop_na(.) %>%
  droplevels(.)
...
weightdiff_mm <- lme(fixed = DifferencePoids ~ TeaType + C_over_N + NitrogenType + DoseN_sc*TeaType,
...
omdecomp_data = donnee %>%
  select(TauxDecomposition_MO, TeaType, DoseN, Producteur,Champs, C_over_N, DoseRep,NitrogenType) %>%
  drop_na(.) %>%
  droplevels(.)
...
omdecomp_mm = lme(fixed = TauxDecomposition_MO ~ TeaType + C_over_N+  NitrogenType + DoseN_sc*TeaType,
                    random = ~ 1 | Producteur/Champs/DoseRep,
                    data = omdecomp_data)
summary(omdecomp_mm)
anova(omdecomp_mm)
...
aggregate(Pourcentage_pertePoids ~ TeaType + DoseN, data = donnee, FUN = min)
aggregate(Pourcentage_pertePoids ~ TeaType + DoseN, data = donnee, FUN = median)
aggregate(Pourcentage_pertePoids ~ TeaType + DoseN, data = donnee, FUN = max)
aggregate(Pourcentage_pertePoids ~ TeaType + DoseN, data = donnee, FUN = sd)
aggregate(Pourcentage_pertePoids ~ TeaType + DoseN, data = donnee, FUN = mean)
aggregate(Pourcentage_pertePoids ~ TeaType, data = donnee, FUN = mean)
aggregate(Pourcentage_pertePoids ~ TeaType, data = donnee, FUN = sd)



aggregate(Pourcentage_C_perdu ~ TeaType + DoseN, data = donnee, FUN = min)
aggregate(Pourcentage_C_perdu ~ TeaType + DoseN, data = donnee, FUN = median)
aggregate(Pourcentage_C_perdu ~ TeaType + DoseN, data = donnee, FUN = max)
aggregate(Pourcentage_C_perdu ~ TeaType + DoseN, data = donnee, FUN = sd)
aggregate(Pourcentage_C_perdu ~ TeaType + DoseN, data = donnee, FUN = mean)
