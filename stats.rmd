...
map_in <- get_stamenmap(frame_map_zoom_in, zoom = 8, maptype = "toner-lite") %>% # toner-lite
  ggmap() +
  geom_label_repel(data = sites, aes(x = lon, y = lat, label = names), size = 5, nudge_x = 1.5,  nudge_y = 0.01, box.padding = 1.001, label.padding = 0.25, segment.linetype = 3) +
 geom_point(aes(x = lon, y = lat), data = sites, size = 2) +
  labs(x = "Longitude", y = "Latitude", title = "B") +
theme(strip.text = element_text(size = 12), axis.text=element_text(size=10, face = "bold"),
        axis.title=element_text(size=12)) 
map_in
```
